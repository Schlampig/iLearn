...
def main():
    ...
    global_step = 0
    if args.do_train:
        cached_train_features_file = 'features_{0}_{1}_{2}_{3}_{4}.pkl'.format(
            list(filter(None, args.bert_model.split('/'))).pop(), str(args.max_seq_length), str(args.doc_stride), str(args.max_query_length), str(args.train_file.strip(".json").split("/")[-1]))
        train_features = None
        try:
